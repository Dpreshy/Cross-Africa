...
const NewSellers = () => {
  ...
  const {data} = useQuery({
    ...
    const filteredDate = data?.filter((el)=> el.category === id)
  const [ currentPage, setCurrentPage ] = useState(0)
  const recordPage = 6
  const lastIndex = currentPage * recordPage
  const pageCount = Math.ceil(filteredDate?.length / recordPage)
  const currentPageData = filteredDate?.slice(lastIndex, lastIndex + recordPage)
      ...
      <Buttom>
        ...
        </Head>
        {
            searchData?.map((props,index) => (
                <Productpage index={ index } key={ index } avatar={ props?.avatar[ 0 ].url } name={ props.name } tag={ props.tag_No } status={ props?.status } price={ nf.format(props.price) } category={ props.category} />
              // 
            ))
      }
        </Buttom>
