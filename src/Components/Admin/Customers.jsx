...
const NewSellers = () => {
  ...
  return (
      ...
      <Container>
        <Header>
                  <SerachHold>
                  <button onClick={()=>search(search(currentPageData))}>
            <BiSearch />
            </button>
          <input
      ...
      <Buttom>
        ...
        </Head>
        {
            searchData?.map((props,index) => (
              <OrderPage index={ index } id={ props._id } isSeller={ props.isSeller } order_No={ props.phoneNum } registerd={ moment(props.createdAt).format("D MMM YYYY") } country={ props?.country } firstName={props.firstName} lastName={props.lastName}/>
              // 
            ))
      }
        </Buttom>
